...
void RunBookStore() {
    ...
    while (option != -1)
    {
        ...
        if (validationOption.length() >= 1 &&
            validationOption.at(0) == 'j')
        {
            // Step 2*
...
int GetManagementOption()
{
    ...
    while (true)
    {
        ...
        option = GetIntInput();
        if (option == -1 || option == 1 || option == 2) { break; }
...
int FindBookIndex(vector<Book> v, string target_name)
{
    ...
    for (size_t i = 0; i < v.size(); i++)
    {
        if (v.at(i).name == target_name) { return i; }
    }
...
int FindBookIndex(vector<Book> v, int target_id)
{
    ...
    for (size_t i = 0; i < v.size(); i++)
    {
        if (v.at(i).id == target_id) { return i; }
    }
...
string GetStringInput(string requestLine)
{
    ...
    while (true)
    {
        ...
        // if succeeded -> break the loop,
        // otherwise    -> iterate again
        if (rawInput.length() >= 1 && rawInput.at(0) != ' ') { break; }
